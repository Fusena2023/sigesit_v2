...
<body>
	...
	<div class="col-sm-9 col-sm-offset-3 col-lg-10 col-lg-offset-2 main">
		...
		<div class="row">
			<div class="col-lg-12">
				<div class="panel panel-default">
					<div class="panel-heading">List Pertanyaan & Jawaban</div>
				...
				<div class="panel panel-default">
					<div class="panel-heading">Tambah Pertanyaan & Jawaban</div>
					<div class="panel-body">
						<div class="col-md-12">
							<form method="POST" name="formsubmitfaq" action="{{ route('admin.simpanfaq') }}" enctype="multipart/form-data" role="form">
								@csrf
								<div class="row mb-4">
									<div class="col-md-3">
		...
		<script>
			$(function() {
				$('#users-table').DataTable({
					processing: true,
					serverSide: true,
					ajax: "{{ route('admin.datatables.getdatafaq') }}",
					columns: [{
							data: 'id',
							name: 'id'
						},
		...
		<script type="text/javascript">
			function show(id, status, pertanyaan, jawaban) {
				var html2 = '<center><font color="red">' + pertanyaan + ' </font><br>Ingin di ' + status + '</center><form method="POST" name="formshowfaq" action="{{ route("admin.showfaq") }}" role="form">@csrf<input name="id" type="hidden" value="' + id + '"></form>';
				Swal.fire({
					type: 'question',
					title: status + ' FAQ',
					html: html2,
			...
			function edit(id, pertanyaan, jawaban) {
				var html = '<form method="POST" id="formeditfaq" action="{{ route("admin.editfaq") }}" enctype="multipart/form-data" role="form">' +
					'@csrf' +
					'<div class="form-group">' +
					'<label>Pertanyaan</label>' +
					'<textarea name="pertanyaan" class="form-control" rows="3" required>' + pertanyaan + '</textarea>' +
					'<input name="id" type="hidden" class="form-control" value="' + id + '" required>' +
					'</div>' +
					'<div class="form-group">' +
					'<label>Jawaban</label>' +
					'<textarea id="jawaban" name="jawaban" class="form-control" rows="9" required>' + jawaban + '</textarea>' +
					'</div>' +
					'</form>';
			...
			function submitfaqform() {
				...
				var desc = CKEDITOR.instances['jawaban'].getData();
				console.log(desc);
				if (pertanyaan == ''||desc=='') {
					Swal.fire({
						type: 'warning',
